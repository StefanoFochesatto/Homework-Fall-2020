...
\end{enumerate}
\end{exercise}
\begin{proof}[Proof (a)] Suppose $C \subseteq [0,1]$ is uncountable. Suppose the sake of contradiction that for all $a \in (0,1)$,
  $C \cap [a,1]$. Let $a = \frac{1}{n}$. Note that $c \cap [\frac{1}{n},1]$ is countable.  By Theorem 1.5.8 we now hat since $c \cap [\frac{1}{n},1]$ the infinite union is also countable. Through set theory
  \begin{align*}
...
\end{exercise}
\begin{proof}
  Suppose for each $k\in\Nats$ that $A_k$ is at most countable. Recall that since $A_k$ is at most countable and therefore has an imposable total order letting us reference elements by index.  Consider the function $f: \Nats\times\Nats \to \cup_{k=1}^\infty A_k$ defined such that,
  $f(n,m) = A_n[m]$. let $a \in \cup_{k=1}^\infty A_k$ and by definition we know that $a$ must exist in some set $A_i$, where $i \in \Nats$. Furthermore we can index the set $A_i$ and let the index for $a$ be $j \in \Nats$. Therefore we know that $f(i,j) = A_i[j]$ where $i,j \in \Nats$. Thus $f$ is a bijection, and it follows that since
